...
namespace lab{
    ...
    struct PolarCoord {
        float radius, degree;
        PolarCoord(float radius, float degree) : radius(radius), degree(degree) {}
    };
    ...
    struct VertexFormat{
        ...
        VertexFormat(PolarCoord &pxz, float py, PolarCoord &nxz, float ny, float tx, float ty){
            position_x = pxz.radius * cos(pxz.degree); position_y = py; position_z = pxz.radius * sin(pxz.degree);
            normal_x   = nxz.radius * cos(nxz.degree); normal_y   = ny; normal_z   = nxz.radius * sin(nxz.degree);
            texcoord_x=tx;        texcoord_y=ty;
        }
